...
export class DialogasignarempleadoComponent implements OnInit {
  ...
  displayedColumns: string[] = ['nombres', 'stock', 'descripcion', 'asignar'];
  ...
  asignar(empleado) {
    ...
    if (this.cantidad == 0 || this.cantidad == null) {
      ...
      Swal.fire({
        icon: 'warning',
        confirmButtonColor: '#1d1d24',
        text: 'Debe ingresar la descripcion para continuar'
      });
      ...
      this.inventarioEmpleado.descripcion_inv = this.descripcion;
      ...
      if (stockNuevo == 0) {
        Swal.fire({
          icon: 'warning',
          confirmButtonColor: '#1d1d24',
          text: 'Recuerde que ya no dispone de stock!!!!!!'
        });
